...
void writeFont(fstream& plik, uint16_t* font, int height, int width);
...
int main() {
	...
	writeFont(plik1, Font6x8, 8, 6);
	writeFont(plik2, Font7x10, 10, 7);
	writeFont(plik3, Font11x18, 18, 11);
}

void writeFont(fstream& plik, uint16_t* font, int height, int width) {
	for (int i = 0; i < 95; i++) {
		plik << endl << endl << char(i + 32) << endl;
		for (int j = 0; j < height; j++) {
			for (int k = 0; k < width; k++) {
				if (getBit(font[i * width + k], j) == 0)
					plik << "-";
				else
					plik << "#";
			}
